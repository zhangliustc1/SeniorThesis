...
//	public static String hypfile = workdir + "PyNGram_clustered_clusters6_cumulative.txt";
	...
	public HashSet<HashSet<String>> readInClustering(String clusterFilename)
	{
		...
		while(line!=null) {
			...
			if(line.length>0 && line[0].trim().length()>0) {
				...
				// Do this block when clustering by a single string from the extraction (ie, obj, OR rel) 
				//System.out.println(line[0]);
				cluster.add(line[0]);	
				
				// Do this block when clustering entire lines.
	...
	// This sorts DESCENDING (largest first)
										  ...
										  HashSet<HashSet<String>> g)
	{
		...
		// Sort hyp clusters by size (descending)
		LinkedList<HashSet<String>> hypClustersSortedForSize =
			sortClustersForSize(h);
		
		// Loop over hyp clusters, starting with largest
		int hClustSize = 0;
			...
			itHypClusters.hasNext(); ) 
		{
			...
			// Increment totalH, again these only show up
			// in gold clusters also
			totalH += hClustSize;
			numHypClusters++;
		...
		for(Iterator<HashSet<String>> itGoldClusters = g.iterator(); itGoldClusters.hasNext(); ) {
			...
			if(goldToHypMatchSize.containsKey(gclust)) {
				...
				for(Iterator<String> gIt = gclust.iterator(); gIt.hasNext(); ) {
					...
					// If this gold element is NOT in allHypElements
...
//							 String.valueOf(stats[11])};
	...
	private void writeGradedClusterOld(
			...
			BasicDiskTable out)
	{
		...
		// Decide whether or not a blank line should be printed.
		// Runs only one iteration. There is no good way to get one element from a HashSet!
		for(Iterator<String> hIt = hclust.iterator(); hIt.hasNext(); ){
			if(gEls.contains(hIt.next())){
				out.printFlatLine("");
			}
			break;
		}
			...
			String gradedHyp)
	{
		...
		// Sort hypothesis clusters by size, is this ascending??
		LinkedList<HashSet<String>> hypClustersSortedForSize =
			sortClustersForSize(hyp);
		
		// Loop through all hyp clusters, starting with largest
		int hClustSize = 0;
		for(Iterator<HashSet<String>> itHypClusters = hypClustersSortedForSize.iterator(); 
			itHypClusters.hasNext(); ) 
		{
			...
			// Loop through all strings in current hyp cluster
			hClustSize = 0;
			for(Iterator<String> itHyp = hclust.iterator(); itHyp.hasNext(); ) {
				String el = itHyp.next();
				if(allGoldElements.contains(el)) {
...
//			}
			...
			for(Iterator<HashSet<String>> itGClusters = gold.iterator(); itGClusters.hasNext(); ) {
				...
				// Loop over strings in current hyp cluster
				// Find size of intersection with current gold cluster
				for(Iterator<String> itHyp = hclust.iterator(); itHyp.hasNext(); ) {
					if(gclust.contains(itHyp.next())) {
						HGScore++;
					}
...
//		
		...
		BasicDiskTable outbdt = new BasicDiskTable(new File("justdeletemeOK.txt"));
		...
		System.out.println("Size of first sorted hyp cluster");
		LinkedList<HashSet<String>> ll = sortClustersForSize(hyp);
		...
		this.calculatePR(hyp, gold, param, mergeIteration, outbdt, isObjects);
	}
	...
	public static void main(String[] args) {
		LinearPrecisionRecallCalculator lprc = new LinearPrecisionRecallCalculator();
		
//		if (args.length > 0){
//			System.out.println("Running the PR Calculator on " + args[0]);
//			lprc.printResults(goldfile, args[0]);
//		}else{
//			lprc.printResults(goldfile, hypfile);
//		}
		...
		System.out.println(lprc.sortClustersForSize(clusters));
