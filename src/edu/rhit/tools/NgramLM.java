...
public class NgramLM {
	...
	// Assuming bigrams here.
	private float getPStar(String s){
		String word = s.substring(s.length()-1);
		String context = s.substring(0, s.length()-1);
		int n = 2;
		
		HashMap<String, Integer> bigrams = ngramModels.get(2);
		
		int k = 5;
		
		int c = 2;
		
		// Nc+1  = num N-grams that occur c+1 times
		// Nc  = num N-grams that occur c times
//		Nc1 =  
		
		// kterm = (k+1) Nk+1 / N1
		// But what about those n-grams that don't occur more than 6 times?
		
		// c* =( (counts + 1) * (Nc+1)/(Nc) - (counts * kterm ) / (1 - kterm)  
		
		// whole string count with * / context count
